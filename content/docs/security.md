...
The Datadog write-up covers a failure mode the other two don't. Their example, Clawsights, is a real skill found in the wild that poses as a leaderboard for Claude Code, then reads your GitHub token with `gh auth token` and sends it to a server the attacker controls with `curl`, disguised as a stats upload. Run as an ordinary skill, that intent is right there in the text, and when Datadog tested it, Opus 4.6 on its highest reasoning setting read the instructions, flagged the credential theft, and refused.
