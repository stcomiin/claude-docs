...
| --- | --- | --- |
| **OWASP Top 10:2025** | Baseline for every web app. Notable 2025 changes: **Software Supply Chain Failures** (A03, new) and **Mishandling of Exceptional Conditions** (A10, new); SSRF folded into A01 Broken Access Control. | [owasp.org/Top10/2025](https://owasp.org/Top10/2025/) |
...
| **OWASP Top 10 for LLM Applications 2025** | Any feature that calls an LLM (chatbots, RAG, summarisers, agents). Prompt injection is still **#1**; new 2025 entries are **LLM07 System Prompt Leakage** and **LLM08 Vector & Embedding Weaknesses** (RAG). | [genai.owasp.org/resource/owasp-top-10-for-llm-applications-2025](https://genai.owasp.org/resource/owasp-top-10-for-llm-applications-2025/) |
...
| --- | --- | --- |
| **CVE-2025-52882** — WebSocket auth bypass | Unauthenticated WebSocket bound to localhost in the Claude Code VS Code/JetBrains extension (≤ v1.0.23). Visiting a malicious page let attackers run MCP commands — read files, execute Jupyter cells. CVSS 8.8. Patched in v1.0.24. | [Datadog Security Labs](https://securitylabs.datadoghq.com/articles/claude-mcp-cve-2025-52882/) |
| **CVE-2025-54794 / CVE-2025-54795** — "InversePrompt" | Path-restriction bypass and command injection in Claude Code itself. Patched in v0.2.111 and v1.0.20 respectively. | [Cymulate writeup](https://cymulate.com/blog/cve-2025-547954-54795-claude-inverseprompt/) |
| **CVE-2025-59536 / CVE-2026-21852** — "Caught in the Hook" | Malicious project files (`.claude/` configs, hooks, MCP definitions, env vars) achieved RCE and exfiltrated API tokens when a victim opened the project. | [Check Point Research](https://research.checkpoint.com/2026/rce-and-api-token-exfiltration-through-claude-code-project-files-cve-2025-59536/) · [The Hacker News](https://thehackernews.com/2026/02/claude-code-flaws-allow-remote-code.html) |
| **"Claudy Day"** — claude.ai trio | Three chained vulns at the platform layer: targeted delivery → invisible prompt manipulation → silent exfiltration of conversation history. | [Oasis Security](https://www.oasis.security/blog/claude-ai-prompt-injection-data-exfiltration-vulnerability) |
| **MCP architectural RCE** | Systemic prompt-injection RCE pattern affecting Cursor, VS Code, Windsurf, Claude Code, Gemini-CLI. 150M+ MCP downloads in scope. | [OX Security](https://www.ox.security/blog/the-mother-of-all-ai-supply-chains-critical-systemic-vulnerability-at-the-core-of-the-mcp/) |
| **Indirect prompt injection via tool output** | Web content / file content fetched by the agent contains hidden instructions that hijack the next turn. | [Lasso Security: Hidden backdoor in Claude](https://www.lasso.security/blog/the-hidden-backdoor-in-claude-coding-assistant) |
| **Claude Desktop Extensions RCE** | 10K+ users exposed via extension RCE. | [LayerX](https://layerxsecurity.com/blog/claude-desktop-extensions-rce/) |

### Sandbox, permissions, and Anthropic's own guidance

- `/sandbox` enables OS-level **filesystem and network isolation** (seatbelt on macOS, bubblewrap on Linux/WSL2). Use it whenever you'd otherwise reach for `--dangerously-skip-permissions`.
