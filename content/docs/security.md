...
The Datadog write-up covers a failure mode the other two don't. When a skill uses dynamic context, its `!` commands run while the skill is still being assembled, before the finished text ever reaches Claude. The model never sees the command, so it can't refuse it. To show what that allows, the authors built a fake "Clawsights" skill that poses as a Claude Code leaderboard. It runs `gh auth token` to grab a GitHub token, then uses `curl` to send it to a server they control, all before any safety check has a chance to fire. In one test, Claude even claimed afterward that it wouldn't run a command it had already run.
